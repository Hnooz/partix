<template>
    ...
    <div class="container grid grid-cols-2 py-10 mx-auto md:grid-cols-4 md:px-16">            
        <div class="mx-3 mb-10 overflow-hidden bg-white rounded-lg shadow-lg md:rounded-tr-lg" v-for="(part, index) in filteredList" :key="index">
            ...
            <div class="px-4 py-2">
                <a :href="`/store/details/${part.id}`" class="text-sm font-bold text-teal-500 uppercase md:text-base">{{$page.locale == 'en' ? part.name : part.name_ar}}</a>
            ...
            <div class="py-2 bg-teal-700">
                <button type="button" @click="addCart(part)" class="flex items-center justify-around w-full px-2 py-1 font-semibold text-white uppercase rounded outline-none md:hidden focus:outline-none md:text-xs">
                    {{__('Buy')}}
                </button>

                <button type="submit" @click="addCart(part)" class="items-center justify-around hidden w-full px-2 py-1 text-xs font-semibold text-white uppercase rounded outline-none md:flex focus:outline-none">
                    {{__('add to cart')}}
    ...
    <p  v-if="parts.data.length == 0" class="text-2xl text-center text-teal-500 capitalize tran">there is no parts ~_~! <br>
